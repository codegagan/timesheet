			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="contact" title="${message(code: 'resource.contact.label', default: 'Contact')}" />
					
						<g:sortableColumn property="email" title="${message(code: 'resource.email.label', default: 'Email')}" />
				...
				<g:each in="${resourceInstanceList}" status="i" var="resourceInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${resourceInstance.id}">${fieldValue(bean: resourceInstance, field: "contact")}</g:link></td>
					
						<td>${fieldValue(bean: resourceInstance, field: "email")}</td>
